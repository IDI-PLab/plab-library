 ...
 * https://www.ntnu.no/wiki/display/plab/6.+Pololu+Zumo+Robot
 * 
 * DS 23.02.2016: Added self calibration at startup. 
 * 
 */
...
const int MAX_SPEED = 400; // Maximum speed of our robot

const int LEFT_LIGHT_INPUT = A4; // Left light input pin
const int RIGHT_LIGHT_INPUT = A5; // Right light input pin

int MIN_LIGHT = 800; // We assume some background light noise
int MAX_LIGHT = 1024; // Maximum possible sensor reading
...
void setup() {
  ...
  int average = (leftReading + rightReading) / 2;
  MIN_LIGHT = average;
  MAX_LIGHT = MIN_LIGHT + 50;
}
